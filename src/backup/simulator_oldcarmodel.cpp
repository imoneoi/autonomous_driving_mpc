...
void Simulator<T>::step(SimulatorState<T> *state, const SimulatorInput<T> &input, const SimulatorParameters<T> &param, int num_steps) {
    ...
    T Kf = param.extforce_parameters.Kf;
    ...
    for (int step = 0; step < num_steps; step++) {
        ...
        T a_F     = std::atan((state->v_y + a * state->r) / (state->v_x + eps)) - d_f;
        T a_R     = std::atan((state->v_y - b * state->r) / (state->v_x + eps));
        ...
        T FyR         = (std::abs(FyR_paj) < std::abs(FyR_max)) ? FyR_paj : FyR_max;
        ...
        //compute friction
        x_force += ( -((state->v_x > 0) ? 1 : -1) * (a0 * state->v_x * state->v_x ) ) + (-Kf * state->v_x);
